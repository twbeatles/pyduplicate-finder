...
Implemented from this backlog in this cycle:
- Results-tree delta selection signal and cached summary updates.
- Main-window selection path now avoids repeated filesystem `getmtime` calls by reusing result metadata.
- Async + cached preview pipeline (controller-based split) to reduce UI stalls.
- Scanner matcher pre-compilation, session progress DB-write throttling, and session-hash batch dedupe.
- Folder-duplicate full-hash stage moved onto the parallel hash pipeline.
- Export path now accepts metadata injection (`file_meta`) to avoid redundant `os.path` stats.
- Added deterministic perf-regression tests and benchmark runner.
